...
count = 0;csscode='',jscode='';

eventHandler.on('before', function(){
...
router.get('*', function(req, res, next) { //设置Content-Type 和检测是否是debug模式
 eventHandler.emit('before');
 var nougjs = noys(req.path);
  cscDebug = config.debug || req.param('cscdebug') ||nougjs|| url.parse(req.get('Referer') || '', true).query.cscdebug;
 
  next();
});

router.get(/^\/(v2|js|css)\/(.[^=]*)\.(cs|j)s$/, function(req, res) { //单文件版本css 和js 处理
...
/*
    ...
    fs.appendFile('txt.txt', code,  function(err) {
   if (err) {
...
console.log("准备写入文件");
fs.writeFile('txt.txt', code,  function(err) {
   if (err) {
       return console.error(err);
   }
   console.log("数据写入成功！");
