...
//地址栏交互
    ...
    var local_st=require('//res.csc86.com/f=v3/dmg/market/js/src/pro_de_intrLstorage');//加载本地存储函数
    ...
    function conmon_dom(ar1,ar2,ar3,ar4){//优惠券部分共用dom函数
      var str= /*优惠券*/
        ...
        str+="<img src='//res.csc86.com/v3/dmg/market/demo/pro_intrSize_choose.png' alt='' class=' sh_v_middle pro_intrSize_choose'>"+
        "</div>"+
        "</div>"+
        "</div>"+
            /*优惠券*/
                ...
                success: function (data) {
                    ...
                    if(data.status==100){
                        if(weixins.isWeiXin()){
                            ...
                            if(type){
                                window.location.href="//res.csc86.com/v3/dmg/market/html/qrcode_payment.html?orderId=" + data.data.buyOrder+"&payMoney="+newdata.allprice;
                            }else{
                                window.location.href ="//res.csc86.com/v3/dmg/market/html/qrcode_payment.html?orderId=" + data.data.addOrder+"&payMoney="+newdata.allprice;
                            }
                        }else{
                        ...
                        window.location.href="//res.csc86.com/v3/dmg/market/html/p_order.html"
                    }
                }
            });
        }
                ...
                if(type=="1"){
                    if(data.status==100){
                        ...
                        if(data.data.goodsToOrder.hasCoupons){
                            $(".coupon_jp").on("click",function(event){//详情页进入快速购买优惠券可用
                                ...
                                window.location.href='//res.csc86.com/v3/dmg/market/html/coupon-use.html?pageId='+type+'&token='+quickBuyToken;
                            })
                        }
                        $('.con_order-userhref').on("click",function(event){ //地址栏点击改变地址
                            window.location.href='//res.csc86.com/v3/dmg/market/html/con_order_addrListPre.html?pageId='+type+'&token='+quickBuyToken;
                        });
                    ...
                    if(data.status==100){
                        ...
                        $('.con_order-userhref').on("click",function(event){ //购物车前来地址栏点击改变地址
                            ...
                            window.location.href='//res.csc86.com/v3/dmg/market/html/con_order_addrListPre.html?pageId='+type+'&token='+token;
                        });
                        if(data.data.goodsToOrder.hasCoupons){
                            $(".coupon_jp").on("click",function(event){ //购物车前来优惠券可用
                                event.preventDefault();
                                local_st.set("cart-argument",JSON.stringify(argument));
                                window.location.href='//res.csc86.com/v3/dmg/market/html/coupon-use.html?pageId='+type+'&token='+token;
                            })
                        }
                        $(".con-order-submit").on("click",function(event){//确认订单购物车进入提交按钮
                            ...
                            event.preventDefault();
                            submit_orderAjax( '//'+hostmap.test+'/order/addOrder',{addressId:addressId,token: token,confirmOrderToken:token,lineIds:lineIds,cpId:data.data.goodsToOrder.cpId,cuproduct:data.data.goodsToOrder.cuproduct},"//res.csc86.com/v3/dmg/market/html/pro_de_cart.html",false,data.data.goodsToOrder,newdata)
                        })
                    }else if(data.status==102){
