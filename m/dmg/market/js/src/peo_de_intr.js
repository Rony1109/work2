define(function (require, exports, module) {
    require('swiper');//滑动插件
    require('//res.csc86.com/f=v3/dmg/market/js/src/tab')(".sh_tab_box li",".sh_tab",true);//tab切换
    var formatNum=require('//res.csc86.com/f=v3/dmg/market/js/src/formatNum');//加载金额三位数一逗号函数
    var isLogin = require('//api.csc86.com/notify/count/all/?callback=define');//加载登录接口
    var dialog=require('layer');//弹框插件
    var cart_show=require('//res.csc86.com/f=v3/dmg/market/js/src/isLogin_not');//加载购物车数量显示函数
    $.debug=false;
        ...
        size_choose: function (shangPin) {//SKU区域方法汇总
                ...
                $.get(url,params, function(data) {
                    ...
                    if(type){
                        ...
                        if(data.status==100){
                            ...
                            _hide();
                            window.location.href="//res.csc86.com/v3/dmg/market/html/con_order.html?pageId=1&token="+shangPin.token
                        }else{
                            tips("<label class='sh_font_sz32'>"+data.msg+"</label>");
                            _hide();
                        }
                    ...
                    success: function (data) {
                        if(type){
                            if(data.status==200){
                                ...
                                $(".pro_intrStore_ch").eq(0).attr("src", "//res.csc86.com/v3/dmg/market/demo/pro_intrStore_f.png");
                                tips("<label class='sh_font_sz32'>"+data.msg+"</label>");
                                _is_store_not=false;
                                return;
                            }else{
                            ...
                            if(data.status==200){
                                ...
                                $(".pro_intrStore_ch").eq(0).attr("src", "//res.csc86.com/v3/dmg/market/demo/pro_intrStore_l.png");
                                tips("<label class='sh_font_sz32'>"+data.msg+"</label>");
                                _is_store_not=true;
                                return;
                            }else{
            ...
            function _change_store() {//添加到收藏夹功能
                if ($.debug||isLogin.status){
                    ...
                    $(this).attr("src", $.debug ? "../demo/pro_intrStore_f.png":"//res.csc86.com/v3/dmg/market/demo/pro_intrStore_f.png");
                    window.location.href = $.debug ? "../market/html/login.html":"//res.csc86.com/v3/dmg/market/html/login.html";
                }
            }
            function _store(){//判断是否收藏
                $.ajax({
                    ...
                    success: function (data) {
                        if(data.status==200){
                            $(".pro_intrStore_ch").eq(0).attr("src", $.debug ? "../demo/pro_intrStore_l.png":"//res.csc86.com/v3/dmg/market/demo/pro_intrStore_l.png");
