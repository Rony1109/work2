    ...
    var p_center={
            p_center_jump:function(){
             function p_c_jump(c_ele,jump_pag){//个人中心点击各个按钮页面跳转函数
                ...
                p_c_jump(".p_center_str","//res.csc86.com/v3/shopping_center/market/html/p_collect.html");//我的收藏页面调整
                p_c_jump(".p_center_ord","//res.csc86.com/v3/shopping_center/market/html/p_order.html");//我的订单页面跳转
                p_c_jump(".p_center_addr","//res.csc86.com/v3/shopping_center/market/html/con_order_addrList.html");//地址管理页面跳转
                p_c_jump(".p_center_acc","//res.csc86.com/v3/shopping_center/market/html/p_centerAccSetting.html");//个人账户设置跳转
                p_c_jump(".p_center_coupon","//res.csc86.com/v3/shopping_center/market/html/coupon-active.html");//优惠券跳转
           function _isLogin_center(conele){ //是否登录banner渲染函数
               ...
               if ($.debug||isLogin.status){
                   ...
                   $(".p_center_resg").on('click',function(event){
                       event.preventDefault();
                       window.location.href="//res.csc86.com/v3/shopping_center/market/html/login_reg_acc.html"
                   })
               }
           }
            ...
            $(".exist_login").on('click', function (event){ //账户设置页面退出登录函数
                ...
                $.get("//"+hostmap.test+"/member/signout",function(data) {
                    if(data.status==200){
                        ...
                        window.location.href="//res.csc86.com/v3/shopping_center/market/html/p_center.html"
                    }else{
