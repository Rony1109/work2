define(function(require, exports, module) {
	$(function(){
		...
		require('swiper');//滑动插件
		require('//res.csc86.com/f=v3/shopping_center/market/js/src/isLogin_not')("//"+ hostmap.test +"/carCount",".sh_cart_index",".pro_de_add_s");//购物车数量显示
		...
		$(".index_ser_bg").on('click',function () {//首页搜索框点击跳转
			...
			if (data_index == "商品") {
				window.location.href ="//"+hostmap.test+"/search/searchProduct.ftl?type=1"
			} else {
				window.location.href = "//"+hostmap.test+"/search/searchProduct.ftl?type=2"
			}
		});
		function index_ser_goo(ele,sel){//商铺、商品搜索页面失焦时跳转页面以及提示函数
			...
			if(null_r!=""&&!reg.test(null_r)){
				...
				if (data_ser == "商品"){
					window.location.href = "//"+hostmap.test+"/search/product.ftl?keyWord=" + null_r
				}else{
					window.location.href = "//res.csc86.com/v3/shopping_center/market/html/index_ser_shopList.html?keyWord=" + null_r
				}
			}else{
				if (data_ser == "商品") {
