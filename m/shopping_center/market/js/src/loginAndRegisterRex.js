        ...
        sign.prototype.submitreg=function (url,eleOpr,content,contentShow,eleOprFinish,type){//========================================================
           ...
           if(type!="login_newPhone_bind"){
               if(!l&&!$pwd) {
                   ...
                   return
               }else if(($pwd&&l<8)||($pwd&&l>20)){
                   flag=tips("<label class='sh_font_sz32'>密码长度为8-20位，区分大小写</label>");
                   return
               }else{
                   ...
                   if(data.status==200){
                       ...
                       tips("<label class='sh_font_sz32'>"+contentShow+data.msg+"!"+"</label>");
                        switch (type) {
                            case "login_reg_acc"://注册新用户
                                ...
                                _success_jumpTo("//res.csc86.com/v3/shopping_center/market/html/login.html?jumpOrNot=0",2000);
                                break;
                            ...
                            case "login_phone_check"://手机号验证（旧手机号换成其他新手机号码）旧手机验证，验证成功后跳转login_bind_tel页面--来源账户设置
                                ...
                                _success_jumpTo("//res.csc86.com/v3/shopping_center/market/html/login_bind_tel.html",2000);
                                break;
                            case "login_newPhone_bind"://微信登陆界面新旧用户验证（手机号是否已注册）没有注册则跳转login_wechat_check页面
                                _success_jumpTo("//res.csc86.com/v3/shopping_center/market/html/login_wechat_check.html?phoneNumber=" + $.trim($("#phone").val()) +"&verificationCode="+$.trim($("#rcode").val())+ "&codeId=" +codeId +"&newUser=200"+"&refUrl="+refUrl,2000);
                                break;
                            case "login_bind_tel":// 修改手机号绑定（换成其他新手机号码）新手机验证--来源旧手机号验证页面
                                ...
                                _success_jumpTo("//res.csc86.com/v3/shopping_center/market/html/p_centerAccSetting.html",2000);
                                break;
                            default:
                                break;
                        }
                       ...
                       switch (type) {
                           case "login_newPhone_bind"://微信登陆界面新旧用户验证成功后跳转页面
                            _success_jumpTo("//res.csc86.com/v3/shopping_center/market/html/login_wechat_check.html?phoneNumber="+ $.trim($("#phone").val()) +"&oldUser=100"+"&refUrl="+refUrl,2000);
                       }
                      }else{
                          tips("<label class='sh_font_sz32'>"+data.msg+"</label>");
                           $(eleOpr).prop("disabled", false).val(eleOprFinish);
