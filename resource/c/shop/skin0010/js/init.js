define(function(require, exports, module){
	...
	var isSubmit=false;
	var inVisible=require('m/inVisible/inVisible.js');
	...
	var Wp={
		...
		index:function(){
			...
			require.async('cscSwitch',function(){
				...
				//橱窗产品
				var $cqScroll=$('.cqres-scroll');
				var $cqResLst=$cqScroll.find('.cqres-lst');
				var $cqLi=$cqResLst.find('li');
				var cqSteps=1;
				var cqVisible=4;
				var cqLiLen=$cqLi.length;
				var showIndex;
				var $cqImg,$cqOtherImg;

				if(cqLiLen>4){
					$cqScroll.cscSwitch($cqLi,{
						effect:'scroll',
						steps:cqSteps,
						visible:cqVisible,
						speed:.6,
						...
						beforeSwitch:function(i,n){
							triggerEventNum++;
							triggerEventArry=[];
							if((n+cqVisible)<=cqLiLen){
								showIndex=n+(cqVisible-1)
							}else{
								showIndex=n-(cqLiLen-cqVisible+1);
							}
							if(showIndex<3){
								/*$cqResLst.find('img:lt('+(3-showIndex)+')').show();
								$cqResLst.find('img:gt('+(cqLiLen-1-(3-showIndex)-1)+')').show();*/
								$cqImg=$('.cqres-lst img:lt('+(3-showIndex)+'),.cqres-lst img:gt('+(cqLiLen-1-(3-showIndex)-1)+')');
							}else{
								/*$cqResLst.find('img').eq(showIndex).show();
								$cqResLst.find('img').eq(showIndex-3).show();
								$cqResLst.find('img:lt('+showIndex+'):gt('+(showIndex-3)+')').show()*/
								$cqImg=$('.cqres-lst img:eq('+showIndex+'),.cqres-lst img:eq('+(showIndex-3)+'),.cqres-lst img:lt('+showIndex+'):gt('+(showIndex-3)+')');
							}
							$cqImg.show();
							$cqOtherImg=$cqResLst.find('img').not($cqImg);
							...
							if(isInVisible) {
								if (typeof cscgaMd == 'object') {
									triggerEventArry.push({id: cscgaMd.getUrlFileName($cqResLst.find('img').eq(showIndex).parents('a:first').attr('href'))});
				...
				$cqResLst.on('mouseenter','li',function(){
					$(this).addClass('hover');
				}).on('mouseleave','li',function(){
					$(this).removeClass('hover');
				});
