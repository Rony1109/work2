define(function(require, exports, module){
	...
	var Wp={
		...
		index:function(){
			...
			//橱窗产品
			var $cqSlide=$('.cqres-slide'),
				$cqSlideBd=$cqSlide.find('.slide-bd'),
				$cqList=$cqSlide.find('.cqres-lst'),
				$cqTrg=$('.cqres-trg'),
				cqLiW=$cqList.find('li').width(),
				cqLiLen=$cqList.find('li').length,
				cqPage=1;
			$cqList.find('img:gt(0)').hide;
			$cqTrg.on('click','li',function(){//点击触点切换图片
				var $this=$(this);
				var index=$('.cqres-trg li').index(this);
				var $curCqImg=$cqList.find('img').eq(index);
				triggerEventArry=[];
				triggerEventNum++;
				$curCqImg.show();
				$cqList.stop(true,false).animate({left : -cqLiW*index},500,function(){
					$cqList.find('img').not($curCqImg).hide();
					if(typeof cscgaMd == 'object'){
						triggerEventArry.push({id:cscgaMd.getUrlFileName($curCqImg.parents('a:first').attr('href'))});
						cscgaMd.commodityExposure.noScrollEvent('pc',triggerEventNum,triggerEventArry);
					}
				});
				$cqTrg.find('li').removeClass('cur');
				$this.addClass('cur');
				cqPage=index+1;
				return false;
			});
			$cqSlide.on('click','.prev',function(){//点击向前按钮
				var $curCqImg,$lastCqImg;
				triggerEventArry=[];
				triggerEventNum++;
				if(cqPage==1){//当到达第一个时，在点击向后按钮则返回显示最后一个
					$cqList.css('left',-cqLiW);
					$cqList.prepend($cqList.find('li:last').clone());
					$curCqImg=$cqList.find('img').eq(cqPage-1);
					$curCqImg.show();
					$cqList.stop(true,false).animate({left:0},500,function(){
						$lastCqImg=$cqList.find('img:last');
						$cqList.css('left',-cqLiW*(cqLiLen-1));
						$cqList.find('li:first').remove();
						$cqTrg.find('li').removeClass('cur');
						$cqTrg.find('li').eq(cqLiLen-1).addClass('cur');
						$lastCqImg.show();
						$cqList.find('img').not($lastCqImg).hide();
						if(typeof cscgaMd == 'object'){
							triggerEventArry.push({id:cscgaMd.getUrlFileName($lastCqImg.parents('a:first').attr('href'))});
							cscgaMd.commodityExposure.noScrollEvent('pc',triggerEventNum,triggerEventArry);
						}
						cqPage=cqLiLen;
					});
					...
					$cqList.stop(true,false).animate({left:'+='+cqLiW},500,function(){
						$cqList.find('img').not($curCqImg).hide();
					});
					cqPage--;
					$cqTrg.find('li').removeClass('cur');
					$cqTrg.find('li').eq(cqPage-1).addClass('cur');
					$curCqImg=$cqList.find('img').eq(cqPage-1);
					$curCqImg.show();
					if(typeof cscgaMd == 'object'){
						triggerEventArry.push({id:cscgaMd.getUrlFileName($curCqImg.parents('a:first').attr('href'))});
			...
			$cqSlide.on('click','.next',function(){//点击向后按钮
				var $curCqImg,$frstCqImg;
				triggerEventArry=[];
				triggerEventNum++;
				if(cqPage==cqLiLen){//当到达最后一个时，在点击向后按钮则返回显示第一个
					$cqList.append($cqList.find('li:first').clone());
					$curCqImg=$cqList.find('img').eq(cqPage-1);
					$curCqImg.show();
					$cqList.stop(true,false).animate({left:-cqLiW*cqLiLen},500,function(){
						$frstCqImg=$cqList.find('img:last');
						$cqList.css('left',0);
						$cqList.find('li:last').remove();
						$cqTrg.find('li').removeClass('cur');
						$cqTrg.find('li').eq(0).addClass('cur');
						$frstCqImg.show();
						$cqList.find('img').not($frstCqImg).hide();
						if(typeof cscgaMd == 'object'){
							triggerEventArry.push({id:cscgaMd.getUrlFileName($frstCqImg.parents('a:first').attr('href'))});
							cscgaMd.commodityExposure.noScrollEvent('pc',triggerEventNum,triggerEventArry);
						}
						cqPage=1;
					});
					...
					$cqList.stop(true,false).animate({left:'-='+cqLiW},800,function(){
						$cqList.find('img').not($curCqImg).hide();
					});
					cqPage++;
					$cqTrg.find('li').removeClass('cur');
