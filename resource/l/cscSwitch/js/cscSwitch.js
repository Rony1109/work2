...
;(function($) {
	...
	$.cscSwitch = {
		cfg: {
			...
			api: false,
			nextBtn:null, /*向后按钮(可直接传入按钮的唯一类名，或者选择器对象)*/
			prevBtn:null, /*向前按钮(可直接传入按钮的唯一类名，或者选择器对象)*/
			nextNo:'', /*向后不在有内容时给按钮添加的类名*/
			prevNo:'', /*向前不在有内容时给按钮添加的类名*/
	...
	function switchTo(triggers,panels, cfg) { 
		var self = this,
			...
			$nextBtn,$prevBtn;
		if(nextBtn){
			$nextBtn=typeof(nextBtn)=='string'?$(nextBtn):nextBtn;
		}
		if(prevBtn){
			$prevBtn=typeof(prevBtn)=='string'?$(prevBtn):prevBtn;
		}
		...
		$.extend(this, {
			click: function(i, e) {
				...
				if(!cfg.circular&&nextBtn&&i>=self.getLength()-1)
				{
					$nextBtn.addClass(cfg.nextNo);
				}
				
				if(!cfg.circular&&prevBtn&&i<=0)
				{
			...
			if ( cfg.triggerType === "mouse" ) {//响应鼠标悬浮
				$(this).bind({
					"mouseenter": function(e) {
						...
						if ( i !== current ) {
							//延时处理，鼠标快速滑过不触发
							switchTimer = setTimeout(function(e) {
								self.click(i, e);
							}, cfg.delay * 1000);
						}
					},
