$(function(){
    ...
    $('.nav .itm1').on('click',function(){
        ...
        $dialogLoading.show();
        $.get("//api.csc86.com/Couponscg/index",{couponId:'20170315001'},function (data){
            $dialogLoading.hide();
            if(data.status){
                var code=data.data.Code;
                if(code==="1"){
                    $yhqDialog2.addClass('show');
                }
                else if(code==="3"){
                    $yhqDialog3.find('.p1').html('对不起，该优惠券只向新人发放！');
                    $yhqDialog3.addClass('show');
                }else{
                    $yhqDialog3.find('.p1').html(data.data.Msg);
                    $yhqDialog3.addClass('show');
                }
            }else {
                $yhqDialog1.addClass('show');
            }
        },"jsonp");
    });
    ...
    $('.kbms-tab li').on('click',function(){
        ...
        $('.kbms-c').eq(index).addClass('kbms-c-cur').siblings('.kbms-c').removeClass('kbms-c-cur');
    });
