		...
		**/
		scrolling:function(options){
			var opts={
				...
				minCycle:6,//为跑的圈数
				result:3,//代表转动后停留的位置
				callback:function(){}//代表中奖后的回调函数
			};
			
			opts=$.extend({},opts,options);

			var	$prizeLst=opts.prizeLst,
			...
			$prizeItm.addClass("opacity");
			$prizeItm.eq(0).removeClass("opacity");
			
			(function move(){
				timer=setTimeout(function(){
					if(count<=minCycle){
						$prizeItm.eq(i).addClass("opacity");
						(i==prizeLen-2) && (count++);
						i<prizeLen-1 ? i++ : i=0;
						$prizeItm.eq(i).removeClass("opacity");
												...
												$.ajax({
													...
													success:function(data){
														if(data.status){
															...
															orderPrize.tips('<span style="color:#069f2a;">信息提交成功</span>');
														}else{
