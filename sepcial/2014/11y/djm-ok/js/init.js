  ...
  $("div.demo").find('ul').each(function(){
	...
	$('#submit').on('click',function(){
		...
		if(company !="" && name !="" && tel !="" && email !="" && qq !=""){
			if(formbool){
				formbool=false;
				$.post("http://ec.csc86.com/index.php?m=formguide&c=index&a=show&formid=23&siteid=1&ajax=1",str,function(data){
					if(data.stutas){
					$('.zxbm-bd').hide();
						$('.success').show();
						var demo=$('.demo ul');
...
//幻灯
	...
	function getdata(){
		$.post("http://ec.csc86.com/api.php",{"op":"formInfo","mid":23,"l":20,"p":1},function(data){
				var  arr=[];
				for(var i=0,len=data.length;i<len;i++){
						arr[i]="<li>"+data[i].company+"</li>";
				}
				$(".demo").find("ul").each(function(){
						$(this).html(arr.join('')); 
				})
		},"jsonp");
	}
