define(function(require, exports, module) {
	...
	ImgScroll.prototype = {
		...
		SetLi: function($this) { //设置初始样式
			...
			$me = $this,
			me = this,
			$img = $me.find('img'), LF = 190;
			for (var i = 0; i < Alen; i++) {
				$img.eq(i).css({
					width: Number(WH - 89 * i) + "px",
					height: Number(HT - 68 * i) + "px"
				});
				$me.find("li").eq(i).attr("style", "left:" + (190 - 60 * i) + "px;z-index:" + (999 - i) + ";top:" + (32 * i) + "px");
			};
			for (var i = Alen; i < len; i++) {
				...
				$me.find("li").eq(i).attr("style", "right:" + (80 + 60 * (i - Alen)) + "px;z-index:" + (0 + (i - Alen)) + ";top:" + (64 / (i - Alen + 1)) + "px")
			};
		},
