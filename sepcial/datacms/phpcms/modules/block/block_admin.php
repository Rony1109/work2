...
class block_admin extends admin {
	...
	public function init() {
		...
		if ($_SESSION['roleid'] != 1) {
			$offset = ($page-1) * 20;
			$r = $this->priv_db->select(array('roleid'=>$this->roleid, 'siteid'=>$this->siteid),'name', $offset.',20');
			$blockid_list = array();
			foreach ($r as $key=>$v) {
				$blockid_list[$key] = $v['blockid'];
			}
			$sql =  implode('\',\'', $blockid_list);
			$list = $this->db->listinfo("id in ('$sql')", 'name', $page, 20);
		} else {
			$list = $this->db->listinfo(array('siteid'=>$this->siteid), 'name', $page, 20);
		}
		$pages = $this->db->pages;
	...
	public function block_update() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			if ($this->db->update($sql, array('id'=>$id))) {
				...
				if(preg_match('/^csc86_.*$/', $data['pos'])){
					showmessage(L('operation_success').'<script type="text/javascript">window.top.art.dialog({id:"test"}).close();window.top.art.dialog({id:"test",title:"碎片更新",content:\'<h2>'.L("operation_success").'</h2><span style="fotn-size:16px;">不要忘记进行栏目缓存更新哟</span><br /><ul style="fotn-size:14px;"><li><a href="javascript:;" target="right"  onclick="window.top.art.dialog({id:\\\'test\\\',close:up(\\\'http://www.csc86.com/flush\\\',\\\''.$data['pos'].'\\\',\\\''.$data['name'].'\\\')}).close();">1.更新csc86首页缓存</a></li></ul>\',width:"400",height:"200"});</script>', '','','edit');
				}else{
