...
//推荐列表中搜索及上一页下一页内容
function findTopicRecommend(comm,a,b){
	...
	switch (comm) {
		case "1"://搜索
			if($("#circleName").val()==""){
				...
				url += "isdesc=DESC&orderType=topicCreateTime";
...
function QZJumper(e,o){//圈子活动 列表跳转分页加键盘事件
	...
	if((currKey==13)){findTopicRecommend("3",o);}
}
//搜索回车
function vipRecKeyDown(e){
	var e=e||event;
	var currKey=e.keyCode||e.which||e.charCode;
	if((currKey==13)){findTopicRecommend("1");}
}

//话题推荐状态修改  updateRecommentTopic('65cbcf80-267c-4c77-8e00-34221e03153c','Y','TOPICDETAIL:','5:','1000:')
function updateRecommentTopic(id,zt,wz,px,obj){
	var url = "circleAuditRec.updateCircleTopicRecommend?",
