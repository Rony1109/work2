...
function fileQueueError(file, errorCode, message) {
	...
	switch(errorCode){
		...
		try{msg="您选择的文件超过最大("+this.settings.file_size_limit+")限制，请处理后上传！"}
		catch(e){msg = "您选择的文件大小超过最大限制，请处理后上传！";}
		break;
...
function uploadSuccess(file, serverData) {//上传一张文件成功后的回调
	if(window.console){console.info(serverData);}
}
...
function uploadComplete(file) {
	...
	if(stats.files_queued==0){//上传队列中文件为0时;
		uploadProgressClose();
		this.settings.file_upload_limit == 1 && this.setStats({successful_uploads:0});
	}else{
		this.startUpload();
	}
	/*
