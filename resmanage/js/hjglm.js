...
var upload = function(callback){		
	...
	var ca=function(){	
			...
			$.post('//bops.csc86.com/bops-app/bops/hjgCategory.uploadBackgroundPic',dataobj,function(data){
				if(data.status===true){
...
$('.seolmtable').on('click','.lmsx',function(){
	...
	$.post('//bops.csc86.com/bops-app/bops/hjgCategory.executeFlushCache',function(data){
		if(data.status===true){
...
$('body').on('click','.adddlm',function(){
		var $this=$(this);
			...
			$.post('//bops.csc86.com/bops-app/bops/hjgCategory.loadCategory',function(data){
			if(data.status===0){
				var lm = '';
		...
		'+lm+'\
		 ...
		 <div class="bglmc">类目名称：<input class="hz2" value=""><input class="py2" type="hidden" value=""></div>\
				...
				var dg=art.dialog({
					...
					var sendurl = '//bops.csc86.com/bops-app/bops/hjgCategory.addSeoCategorys';	
					...
					if($this.is('.editlm')){
						sendurl = '//bops.csc86.com/bops-app/bops/hjgCategory.updateSeoCategory';
					...
					$.post(sendurl,dataobj,function(data){
						if(data.status===true){
							...
							$.post('//bops.csc86.com/bops-app/bops/hjgCategory.loadSeoCategoryByCateId',dataobj2,function(data){
								if(data.status===true){
					...
					init:function(){
						...
						$('.jcfl .wk1').on('click','p',function(){
							...
							$.post('//bops.csc86.com/bops-app/bops/hjgCategory.loadCategory',dataobj,function(data){
								if(data.status===0){
...
$('.seolmtable .up,.seolmtable .down').on('click',function(){
	...
	if(isSubmit===true){return false;}//阻止重复点击
							isSubmit=true;
	$.post('//bops.csc86.com/bops-app/bops/hjgCategory.updateSeoCategorySort',dataobj,function(data){
								if(data.status===true){
									location.href=location.href
									}else{
										artDialog.tip(data.body.errorMsg?data.body.errorMsg:'系统错误请重试！',2);
...
$('.seolmtable .del').on('click',function(){
	...
	if(isSubmit===true){return false;}//阻止重复点击
							isSubmit=true;
	$.post('//bops.csc86.com/bops-app/bops/hjgCategory.deleteSeoCategory',dataobj,function(data){
								if(data.status===true){
									location.href=location.href
									}else{
										artDialog.tip(data.body.errorMsg?data.body.errorMsg:'系统错误请重试！',2);
...
$('.seolmtable').on('click','.edit',function(){
	...
	$.post('//bops.csc86.com/bops-app/bops/hjgCategory.loadSeoCategoryByCateId',dataobj,function(data){
								if(data.status===true){		
								...
								var searchurl = '<a target="_blank" class="dturl" href="'+data.body.searchUrl+'">点击这里</a>';
								...
								var sfgl =  '<span class="cxgl adddlm editlm">重新关联</span>';
		...
		<td class="jsyqlj">'+friendlinkshtml+'</td>\
				...
				$.post('//bops.csc86.com/bops-app/bops/hjgCategory.updateSeoCategory',dataobj,function(data){
		...
		$.post('//bops.csc86.com/bops-app/bops/hjgCategory.loadLinkByLinkId',dataobj,function(data){
			if(data.status===true){
		...
		var dataobj = {"linkName":$linkname,"sort":$sort,"linkUrl":$linkurl,"linkId":$thislinkid,"catSeoId":$thistrmid};	
			...
			if(isSubmit===true){return false;}//阻止重复点击
							isSubmit=true;
	$.post('//bops.csc86.com/bops-app/bops/hjgCategory.updateLink',dataobj,function(data){
								if(data.status===true){
									
			var dataobj = {"catSeoId":$thistrmid};		
			$.post('//bops.csc86.com/bops-app/bops/hjgCategory.loadLinkByCateId',dataobj,function(data){
			if(data.status===true){
				var newlinkhtml='';
	...
	$('.xglmtable').on('click','.yqlj em',function(){
		...
		var  dataobj = {"linkId":$thislinkid};
		$.post('//bops.csc86.com/bops-app/bops/hjgCategory.deleteLink',dataobj,function(data){
			if(data.status===true){
				$thisspan.remove();
				}else{
				artDialog.tip(data.body.errorMsg?data.body.errorMsg:'系统错误请重试！',2);	
	...
	$('.xglmtable').on('click','.add',function(){
		...
		var dataobj = {"linkName":$linkname,"sort":$sort,"linkUrl":$linkurl,"catSeoId":$thistrmid};
			...
			if(isSubmit===true){return false;}//阻止重复点击
							isSubmit=true;
	$.post('//bops.csc86.com/bops-app/bops/hjgCategory.addLink',dataobj,function(data){
		var dataobj = {"catSeoId":$thistrmid};
								if(data.status===true){
		$.post('//bops.csc86.com/bops-app/bops/hjgCategory.loadLinkByCateId',dataobj,function(data){
			if(data.status===true){
...
$('.seolmtable').on('click','.pp',function(){
		...
		$.post('//bops.csc86.com/bops-app/bops/hjgCategory.loadPicByCateId',dataobj,function(data){
			if(data.status===true){
				var zthtml='';
				var sphtml='';
...
<table class="ppzttable"  cellspacing="0" cellpadding="0">\
		...
		$.post('//bops.csc86.com/bops-app/bops/hjgCategory.loadPicByPhotoId',dataobj,function(data){
					if(data.status===true){
...
<table class="addppzttable"  cellspacing="0" cellpadding="0">\
			...
			$.post('//bops.csc86.com/bops-app/bops/hjgCategory.updatePic',dataobj,function(data){
					if(data.status===true){	
	...
	$('.ppzttable').on('click','.s1',function(){
		...
		var  dataobj = {"photoId":$thisphotoid};
		$.post('//bops.csc86.com/bops-app/bops/hjgCategory.deletePic',dataobj,function(data){
			if(data.status===true){
				$thisspan.remove();
				}else{
				artDialog.tip(data.body.errorMsg?data.body.errorMsg:'系统错误请重试！',2);	
...
<table class="addppzttable"  cellspacing="0" cellpadding="0">\
				...
				$.post('//bops.csc86.com/bops-app/bops/hjgCategory.addPic',dataobj,function(data){
					if(data.status===true){
