define(function(require, exports, module) {
  ...
  function sort(data) {
    $.post('app.venueUpdateSort', data, function(data) {
      if (data.success === 'true') {
        dialog.success('排列成功！');
        setTimeout(function() {
          location.href = location.href;
  ...
  $list.on('click', 'a.up', function(event) {
    ...
    var downSort = parseInt($tr.find('td:eq(4)').text(), 10);
    data.upVenueId = $tr.data('id');
    data.upSort = downSort - 1;
    data.downSort = downSort;
    data.downVenueId = $tr.prev().data('id');
    sort(data);
  ...
  $opt.on('click', 'input:button', function(event) {
    event.preventDefault();
