...
define(function(require, exports, module) {
	...
	(function(){
		...
		$('input[name^="RadioGroup"]').on('change',function(){
			...
			if(len>1){
				...
				$.post('updateIsshowSubmit.AdvertisingTwoManage',{advertisingId:id,isShow:val},function(data){
					var msg=data.msg;
	...
	$('.jsDelAll').on('click',function(){
		...
		dialog.confirm("确定要删除选中图片！",function(){
			$.post('bacthDeleteSubmit.AdvertisingTwoManage',{advertisingIds:arryId.join()},function(data){
				var msg=data.msg;
				if(data.success==='true'){
	...
	$('.jsDelOne').on('click',function(){
		...
		dialog.confirm("确定要删除该图片！",function(){
			$.post('deleteSubmit.AdvertisingTwoManage',{advertisingId:id},function(data){
				var msg=data.msg;
				if(data.success==='true'){
					dialog.success(msg,2,function(){
						location.href=location.href;
