<div class="member-info">
	...
	<ul class="mi-basic">
		...
		<li class="g-c-f">
			<strong>创建时间：</strong>
			<p><?php echo $enterprise['createTime'];?></p>
		</li>
	</ul>
	...
	<?php if(ShopHelper::XmlCallBack('joinedCircles')){$tag = 1 ;?>
		...
		<div class="group">
			...
			<?php foreach (ShopHelper::XmlCallBack('joinedCircles') as $value){?>	
				<li><a href="http://quan.csc86.com/index.html?circleId=<?php echo CSCHelper::setDefault($value['circleId'])?>" target="_blank"><img src="<?php echo ShopHelper::image($value['circleLogoUrl']);?>" title="<?php echo CSCHelper::setDefault($value['circleName'])?>" width="80" height="80" /><br/><?php echo CSCHelper::substr($value['circleName'],5)?></a></li>
			<?php }?>
			</ul>
		</div>
		<?php }?>
