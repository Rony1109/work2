<div class="box-sub company-info">
  ...
  <div class="inner">
    ...
    <h2 class="g-c-f busi-name"><strong title="<?php echo $enterprise['enterprise'];?>">
      <?php echo CSCHelper::substr($enterprise['enterprise'],7);?>
      </strong>
    ...
    <ul class="mi-basic">
      <li class="g-c-f"><strong>主　　营：</strong>
        <p>
          <?php echo CSCHelper::substr($enterprise['sellList'],8);?>
        </p>
      </li>
      <li class="g-c-f"> <strong>创建时间：</strong>
        <?php
    ...
    <div class="bot_outer">
      <div class="m-shop-contact">
        <?php if(ShopHelper::XmlCallBack('getContact')){$tag = 1 ;?>
        <?php foreach (ShopHelper::XmlCallBack('getContact') as $value){?>
        <ul class="msc-bd  <?php if(isset($tag)&&$tag==1)echo "cur"?> <?php if(isset($tag)&&$tag == 2)echo "msc-bd2"?>">
          <li> 联系人：
            ...
            <?php if(CSCHelper::setDefault($value['qqNo']) ){?>
            <a target="_blank" href="http://wpa.qq.com/msgrd?v=3&uin=<?php echo CSCHelper::setDefault($value['qqNo'])?>&site=qq&menu=yes"> <img border="0" width="21" height="21" src="<?php echo RES_URL ?>/image/m/third-im/qq.gif" alt="点击这里给我发消息" title="点击这里给我发消息"/> </a>
            <?php }?>
            <?php if(CSCHelper::setDefault($value['wangwang']) ){?>
            <a target="_blank" href="http://www.taobao.com/webww/ww.php?ver=3&touid=<?php echo urldecode(CSCHelper::setDefault($value['wangwang']));?>&siteid=cntaobao&status=2&charset=utf-8"> <img border="0" width="21" height="21" src="<?php echo RES_URL ?>/image/m/third-im/ww.gif" alt="点击这里给我发消息" /></a>
            <?php }?>
          </li>
          <?php if(CSCHelper::setDefault($value['moblie'])){?>
          <li>手　机：
            <?php echo CSCHelper::setDefault($value['moblie'])?>
          </li>
          <?php }?>
          <?php if(CSCHelper::setDefault($value['telephone'])){?>
          <li title="<?php echo CSCHelper::setDefault($value['telephone'])?>">座　机：
            <?php echo CSCHelper::setDefault($value['telephone'])?>
          </li>
          <?php }?>
          <?php if(CSCHelper::setDefault($value['fax'])){?>
          <li title="<?php echo CSCHelper::setDefault($value['fax'])?>">传　真：
            <?php echo CSCHelper::setDefault($value['fax'])?>
          </li>
          <?php }?>
        </ul>
        <?php $tag++;}if($tag ==3){?>
    ...
    <div class="group">
      ...
      <ul class="g-c-f">
        ...
        <li><a href="http://quan.csc86.com/index.html?circleId=<?php echo CSCHelper::setDefault($value['circleId'])?>" target="_blank"><img src="<?php echo ShopHelper::image($value['circleLogoUrl']);?>" title="<?php echo CSCHelper::setDefault($value['circleName'])?>"  width="80" height="80" />
          <?php echo CSCHelper::substr($value['circleName'],5)?>
          </a></li>
        <?php }?>
      </ul>
    </div>
