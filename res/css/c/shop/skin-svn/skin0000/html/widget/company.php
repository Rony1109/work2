<div class="member-info">
	...
	<ul class="mi-basic">
		...
		<li class="g-c-f">
			<strong>创建时间：</strong>
			<p><?php eo($this->shopinfo, 'createTime')?></p>
		</li>
	</ul>
	...
	<?php if($this->getList('joinedCircles')){$tag = 1 ;?>
		...
		<div class="group">
			...
			<?php foreach ($this->getList('joinedCircles') as $value){?>	
				<li><a href="http://quan.csc86.com/index.html?circleId=<?php eo($value, 'circleId')?>" target="_blank"><img src="<?php eo($value, 'circleLogoUrl')?>" title="<?php eo($value, 'circleName')?>" width="80" height="80" /><br/><?php eo($value, 'circleName',5)?></a></li>
			<?php }?>
			</ul>
		</div>
		<?php }?>
