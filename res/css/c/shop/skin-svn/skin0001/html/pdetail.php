<div class="g-o-a head">
	...
	<ul class="g-f-r follow-fav">
		<li class="ff-follow"><em><?php eo($this->shopinfo, 'enpCircleMemberNum')?></em>个圈友<span>
		<a href="<?php echo QUAN_URL.'/index.html?circleId='.setDefault($this->shopinfo, 'enpCircleId')?>"  target="_blank" class="g-ico <?php if (setDefault($this->myself, 'joinEnpCircleFlag')){echo 'ico-followed';}else{echo 'ico-follow';}?>" rel="nofollow"><?php if (setDefault($this->myself,'joinEnpCircleFlag')){echo '已加入';}else{ echo '加入圈';}?></a></span></li>		
		<li class="ff-fav"><em><?php eo($this->shopinfo, 'likeNum')?></em>人喜欢<span><a href="javascript:void(csc.shop.signLike('<?php eo($this->shopinfo, 'id')?>'));" class="g-ico ico-fav<?php if (setDefault($this->myself, 'likedFlag')) echo ' ico-faved';?>" rel="nofollow"><?php if (setDefault($this->myself, 'likedFlag')){echo '已喜欢';}else{echo '我喜欢';}?></a></span></li>
	</ul>
</div>
<ul class="g-o-a nav">
...
<script>
seajs.use(csc.url("res","/f=js/m/statusTab"),function (){
	csc.shop.navCate("ul.nav");
});
</script><div class="g-h-10"></div>

<div class="g-o-a">
	<div class="main product-detail">
		<ol class="crumb">
		<?php if(setDefault($this->pdetail, 'cusCats')){foreach (setDefault($this->pdetail, 'cusCats') as $value){?>
			<li><a href="/scate/<?php eo($value, 'id')?>.html" target="_blank"><?php eo($value, 'name')?></a>&gt;</li>
		<?php }}?>
			<li><strong><?php eo($this->pdetail, 'title')?></strong></li>
		</ol>
		<div class="detail-info">
			<h1 class="di-hd"><?php eo($this->pdetail, 'title')?></h1>
			<div class="g-c-f">
				<div class="g-f-r di-params">
					...
					<table class="di-price">
						<thead>
							<tr>
								<th>订货量（<?php eo($this->pdetail, 'unitsVale')?>）：</th>
								<td>价格：</td>
							</tr>
						</thead>
						<tbody>
						...
						<?php if(setDefault($this->pdetail, 'pricearr')){foreach (setDefault($this->pdetail, 'pricearr') as $value){?>
							<tr>
								<th>≥<?php eo($value, 'more');?></th>
								<td> <strong><?php eo($value, 'detail');?></strong> 
									元/<?php eo($this->pdetail, 'unitsVale')?>
								</td>
							</tr>
						<?php }}?>
						<?php }else{?>
					...
					<div class="di-attr">
						...
						<table width="100%">
							<tr>
								<td>类别：<?php eo($this->pdetail, 'scateName')?></td>
							</tr>
						</table>
					</div>
					<div class="inquiry-fav">
						<a href="http://jiaoyi.csc86.com/inquiry/publish/?proid=<?php eo($this->pdetail, 'id')?>" class="g-f-l" target="_blank" rel="nofollow">立即询盘</a>
						<a href="javascript:void(csc.shop.order('<?php eo($this->pdetail, 'id')?>'));" class="g-f-l di-order" rel="nofollow" data-memberid="<?php eo($this->pdetail, 'memberid')?>">意向订单</a>
				...
				<div class="g-i-v-m di-thumb">
					<div class="thumb-img ibox">
						<a class="i" href="<?php echo IMG_URL.newthumbex(setDefault($this->pdetail, 'pic1'),4)?>" target="_blank">
							<img src="<?php echo IMG_URL.newthumbex(setDefault($this->pdetail, 'pic1'),3)?>" alt="<?php eo($this->pdetail, 'title')?>" data-max="300"/>
						</a>
					</div>
					<ul class="g-c-f thumb-items">
					<?php if(setDefault($this->pdetail, 'picarr')){foreach (setDefault($this->pdetail, 'picarr') as $value){?>
						<li class="ibox">
							<a class="i" href="<?php eo($value, 'imgUrl')?>" target="_blank">
								<img src="<?php eo($value, 'imgUrl')?>" alt="<?php eo($this->pdetail, 'title')?>" data-max="60" />
							</a>
						</li>
					<?php }}?>
					</ul>
			...
			<div class="di-tab-bd">
				<div class="di-tab-bd-item cur detail-intro">
					<div class="un-reset">
				...
				<div class="di-tab-bd-item detail-params">
					<ul class="g-c-f" >
					<?php if(setDefault($this->pdetail, 'productPropertyVOs')){foreach (setDefault($this->pdetail, 'productPropertyVOs') as $value){?>
						<li>
							<span><?php eo($value, 'propertyName')?>:</span>
							<div><?php eo($value, 'propertyvalue')?></div>
						</li>
					<?php }}?>
					</ul>
				</div>
			</div><script>csc.shop.detailThumb();
				seajs.use(csc.url("res","/f=js/m/tab"),function (){
					csc.tab("ul.di-tab-hd>li","div.di-tab-bd>div.di-tab-bd-item","click");
				});</script>
		</div>
	</div>
	<div class="sub">
