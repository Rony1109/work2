...
function _getBasePath() {
	...
	return csc.url('res',"/js/p/kindeditor/4.1.2/");
...
KEditor.prototype = {
	...
	count : function(mode) {
		...
		if (mode === 'img'){
			var imgs = _removeBookmarkTag(_removeTempTag(self.html())).match(/<img.*?>/ig);
			return imgs ? imgs.length : 0;
		}
		if (mode === 'table'){
			var tables = _removeBookmarkTag(_removeTempTag(self.html())).match(/<table.*?[^\/]>/ig);
...
_plugin('core', function(K) {
	var self = this,
        ...
        },shortcutKeys_pass = {"undo":true,"redo":true,"selectall":true,"print":true};
		...
		if (shortcutKeys[name]) {
			self.afterCreate(function() {
				_ctrl(this.edit.doc, shortcutKeys[name], function() {
                    if(!self.options.keyboardShortcut && !shortcutKeys_pass[name]) return false;
                    self.clickToolbar(name);
				});
			});
		}
