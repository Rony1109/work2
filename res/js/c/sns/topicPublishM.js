...
function up10img() {
	...
	artDialog({
		...
		Kmsg_text = KindEditor.create('#topicDescription', {
			themeType : 'qq',
			items : ['bold','italic','fontsize','forecolor','hilitecolor','plug-align','plug-indent','table','link','unlink','emoticons','multiimage'],
			...
			afterChange : function(){
				...
				if(this.count('text')==0||$(".pl_in").val()==""||$(".yzm").val()==""||$(".pl_in").val()=="最多30个字"){
	...
	//发表话题按钮	
	$("input.pl_in,input.yzm").blur(function(){
			if($("input.yzm").val()==""||$("input.pl_in").val()==""||$("input.pl_in").val()=="最多30个字"||Kmsg_text.html()===""||Kmsg_text.html()==="<br>"){
...
function fontLeng(tmp){
	...
	if(Kmsg_text.html()!=""&&$(".pl_in").val()!=""&&$(".yzm").val()!=""){
		$(".pb-btn-21").html('<input type="button" value="提交" class="pb_sub">');
	}else{
		$(".pb-btn-21").html('<input type="button" value="提交" class="pb_b">');
	}
