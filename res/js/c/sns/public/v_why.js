...
verify_why.prototype.patterns = {
	...
	//12小时制时间验证
	time12:{err:{"${time12}请输入有效的24小时制时间格式: 23:00 AM/PM":null},success:{"${time12}": /^((0?\d)|(1[012])):[0-5]\d?\s?[aApP]\.?[mM]\.?$/}},
	//24小时制时间验证
	time24:{err:{"${time24}请输入有效的24小时制时间格式: 23:00":null},success:{"${time24}": /^(20|21|22|23|[01]\d|\d)(([:][0-5]\d){1,2})$/}},
	//阻止HTML标签验证
...
verify_why.prototype.info = function(obj){
	...
	if($(obj).get(0).tagName.toLowerCase() == "form"){
		$(obj).on("form_verifi",function(e,fun){
			fun = fun || function(){};
...
verify_why.prototype.eventinfo = function(obj){
	var o = $(obj), z = this;
	var eve_verifi = obj.data_verifi.eve_verifi || this.eve_verifi;
	o.on("v_verifi",function(){//验证
		return z.v_verifi(this);
	}).on("v_m_err",function(event,msg){//出错提示
		if(!obj.data_verifi.event_err || typeof(obj.data_verifi.event_err) != "function"){
			z.v_m_err(this,msg);
		}else{
			obj.data_verifi.event_err(this,msg);
		}
	}).on("v_m_success",function(event,msg){//通过提示
		if(!obj.data_verifi.event_success || typeof(obj.data_verifi.event_success) != "function"){
			z.v_m_success(this,msg);
		}else{
			obj.data_verifi.event_success(this,msg);
		};
	}).on("v_m_default",function(event,msg){//恢复默认
		if(!obj.data_verifi.event_default || typeof(obj.data_verifi.event_default) != "function"){
			z.v_m_default(this,msg);
		}else{
			obj.data_verifi.event_default(this,msg);
		};
	}).on("v_m_focus",function(event,msg){//焦点提示
		if(!obj.data_verifi.event_focus || typeof(obj.data_verifi.event_focus) != "function"){
			z.v_m_focus(this,msg);
		}else{
			obj.data_verifi.event_focus(this,msg);
		};
	}).on("focus",function(){
		$(this).trigger("v_m_focus");
	}).on(eve_verifi,function(){
