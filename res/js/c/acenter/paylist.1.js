...
$(function(){verifyCheck('#startTime','#endTime','#check_paylist','#checkList')})
function verifyCheck(startime,endtime,form,subtn){
	var startTime = startime , endTime = endtime , Form = form , subBtn = subtn , s , e ,
...
$(function(){
			...
			artDialog({
					...
					init:function(){
						...
						$("#btn_ajsq").click(function(){
							$.get("/account/cancelOrder.html",{"oid":uid},function(data){
								if(data.status){ closet("memtool"); location.reload();}
							},"jsonp");
						});
					}
				});
	...
	$("#fr_sub").on('click',function(){
		if($p.prop('checked') == false){
			...
			var sid = $('input[name="sid"]').val(), itemId = $('input[name="itemId"]:checked').val(), cstcheck = $('input[name="cstcheck"]').val();
			$.get("/service/pay.html",{"sid":sid,"itemId":itemId,"cstcheck":cstcheck},function(data){
				if(data.status){
