$(function(){
	$("#selectArea").delegate(".prov-item:not(.cached)>a","mouseover",function (){
		var
			$this = $(this),
			provinceId = $this.parent(".prov-item").addClass("cached").data('id'),
			cached = false,
			defaultCity;
		seajs.use(csc.url("res","/f=js/m/param"),function (){
			defaultCity = csc.param("city");
		});
		$.each(["北京","上海","天津","重庆","香港","澳门"],function (index,item){
			if($this.text().indexOf(item) > -1){
				cached = true;
			}
		});
		cached || $.get(csc.url("search","/area/getcity.do"),{"provinceId":provinceId},function(data){
	//	cached || $.get("area/getcity.do",{"provinceId":provinceId},function(data){	
			
			if(data.length){
				var str='<ul class="city-items">';
				$.each(data, function(index,v){
					str+='<li'+(defaultCity == v.name ? ' class="cur"' : '')+'><a href="javascript:">' + v.name +'</a></li>';
				});
				str += "</ul>";
				$this.after(str);
			}
...
csc.getArea = function (province,city,defaultProvince,defaultCity){
	...
	$province.on("change",function (){
		var	$t = $(this);
		if($t.val() != "s"){
			$.get(csc.url("search","/area/getcity.do"),{provinceId:$t.val()},function (data){
		//	$.get("area/getcity.do",{provinceId:$t.val()},function (data){
