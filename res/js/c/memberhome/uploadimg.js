...
function fileQueueError(file, errorCode, message) {
	...
	uploadObj.uploadSuccess=function(file,serverData){
		...
		csc.useDialog(function (){
			...
			if(location.pathname == "/" || location.pathname == "" || location.pathname == "/sns/home/friends.html" || location.pathname == "/personal/message/allMessage.html" || location.pathname == "/sns/card.html" || location.pathname == "/user/tool" || location.pathname == "/user/"){
				var url_t = csc.url("img","/scale/160"+response.key); //缩略图地址;
			
				seajs.use(csc.url("res","/f=js/m/cookie"),function (){
					csc.sessionId=csc.getCookie('sessionId');
					$("#infoimg").attr("src",url_t);
					var url = seajs.hostmap.i || 'i.csc86.com';
					$.post("//"+ url +"/updateImg",{"imgUrl":response.key,"sessionId":csc.sessionId},"json");//更新至SNS名片;
				});
